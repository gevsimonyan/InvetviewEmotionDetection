...
# imports for audio preprocessing
import pyaudio
import wave
import librosa
...
class speechEmotionRecognition:
    ...
    def predict_emotion_from_file(self, filename, chunk_step=16000, chunk_size=49100, predict_proba=False, sample_rate=16000):
        ...
        X = mel_spect_ts.reshape(
            mel_spect_ts.shape[0], mel_spect_ts.shape[1], mel_spect_ts.shape[2], mel_spect_ts.shape[3], 1)
        ...
        timestamp = np.concatenate(
            [[chunk_size], np.ones((len(predict) - 1)) * chunk_step]).cumsum()
